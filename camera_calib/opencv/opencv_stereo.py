...
if __name__ == "__main__":
    ...
    while True:
        ...
        left_downsampled = cv2.cvtColor(left_downsampled, cv2.COLOR_BGR2GRAY)
        right_downsampled = cv2.cvtColor(right_downsampled, cv2.COLOR_BGR2GRAY)
        cv2.imshow("left", left_downsampled)
        cv2.imshow("right", right_downsampled)
	
#        cv2.imshow("stereo", composite)
        # if (input() == 's'):
        # cv2.imwrite(f"left/left_{idx}.jpg", left_downsampled)
        # cv2.imwrite(f"right/right_{idx}.jpg", right_downsampled)

        
        key = cv2.waitKey()
        if (key == ord('l')):
            cv2.imwrite(f"left/left_{lidx}.jpg", left_downsampled)
            lidx += 1
        if (key == ord('r')):
